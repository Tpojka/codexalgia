...
def _macos(title, message, sound):
    ...
    display = 'display notification (item 2 of argv) with title "Codexalgia" subtitle (item 1 of argv)'
    if sound:
...
def _linux(title, message, icon, sound):
    command = ["notify-send", "-a", "Codexalgia", "-i", str(icon)]
    if not sound:
...
# CODEXALGIA_AUDIO is one of the two constants below, never user text.
WINDOWS_SCRIPT = r"""
[Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime] | Out-Null
[Windows.Data.Xml.Dom.XmlDocument, Windows.Data.Xml.Dom.XmlDocument, ContentType = WindowsRuntime] | Out-Null
$t = [Security.SecurityElement]::Escape($env:CODEXALGIA_TITLE)
$m = [Security.SecurityElement]::Escape($env:CODEXALGIA_MESSAGE)
$i = [Security.SecurityElement]::Escape($env:CODEXALGIA_ICON)
$xml = New-Object Windows.Data.Xml.Dom.XmlDocument
$xml.LoadXml("<toast><visual><binding template='ToastGeneric'><text>$t</text><text>$m</text><image placement='appLogoOverride' src='$i'/></binding></visual>$($env:CODEXALGIA_AUDIO)</toast>")
$toast = New-Object Windows.UI.Notifications.ToastNotification $xml
[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier($env:CODEXALGIA_APP_ID).Show($toast)
"""
...
def _windows(title, message, icon, sound):
    env = dict(
        os.environ,
        CODEXALGIA_TITLE=title,
        CODEXALGIA_MESSAGE=message,
        CODEXALGIA_ICON=icon.as_uri(),
        CODEXALGIA_APP_ID=WINDOWS_APP_ID,
        CODEXALGIA_AUDIO=WINDOWS_SOUND if sound else WINDOWS_SILENT,
    )
